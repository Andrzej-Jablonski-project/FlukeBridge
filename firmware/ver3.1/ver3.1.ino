...
static const char *FW_VER = "ver3.1";
...
void handleUsbCommands()
{
    ...
    if (line == "PING")
    {
        ...
        Serial.print(" ");
        Serial.print(mode);
        if (ip.length()) { Serial.print(" "); Serial.print(ip); }
        Serial.print("\r");
        return;
    }
    if (line == "VER")
...
String wifiModeStr()
{
    ...
    if (m == WIFI_AP_STA) return "AP+STA";
    return "OFF";
}
...
String prettyFlukeValue(const String &value, const String &unitRaw, const String &status, const String &flags)
{
    ...
    // Parsuj liczbowo (dla większości trybów)
    double v;
    bool ok = toDouble(value, v);

    // Normalizuj unit/status/flags do UPPER
    String u = unitRaw;
    u.trim();
    u.toUpperCase();
    String st = status;
    ...
    // Specjalne tryby wg Fluke:
    // Dioda: część modeli raportuje drop w V (unit=VDC) + flag/status "DIODE"
    ...
    // Ciągłość: zwykle OHM + flag/status "CONT" (próg buzzera zależny od miernika)
    bool isCont = (st.indexOf("CONT") >= 0) || (fl.indexOf("CONT") >= 0) || (st.indexOf("CONTINUITY") >= 0);
    if (isCont && ok)
    {
        // Pokaż BEEP dla bardzo niskiej rezystancji, inaczej format ohm
        if (v <= 50.0)
            return "BEEP"; // domyślny próg ~50 Ω (możesz zmienić)
        return formatOhms(v);
    }
    ...
    // Pojemność (niektóre Fluke podają "FARAD", "F", "CAP")
...
void handleRoot()
{
    ...
    else
    {
        ...
        html += "<p><a href='/status.html'>Dashboard</a> • <a href='/status.json'>status.json</a></p>";
...
void connectOrStartAP()
{
    ...
    {
        WiFi.setHostname("fluke-bridge"); // hostname w sieci
        if (MDNS.begin("fluke-bridge"))
        {                                       // odpowiada na fluke-bridge.local
            MDNS.addService("http", "tcp", 80); // pomocne dla przeglądarek/scanerów
...
<footer>Hold BOOT &gt; 3 s to clear Wi-Fi and reboot (AP: FlukeBridge-XXXX)</footer>
</div>
<script>
async function tick(){
 try{
